            ...
            if(isset($_GET['eliminar'])){
              ...
              if($resultado == "bien"){
                echo '<h2>¡El elemento se ha eliminado!<h2>
                      <form action="'.'http://hadoop.ugr.es:14010'.'" method="'.'get'.'">
                ...
                if(isset($_GET['actualizar']) && isset($_POST['tex'])){
                  ...
                  if(isset($_GET['actualizar'])){
                    echo '<h2>Actualiza el elemento<h2>
                          <form action="'.'http://hadoop.ugr.es:14010/php/muestraResultado.php?actualizar='.$_GET['actualizar'].'" method="'.'post'.'">
                    ...
                    $enlace = new mysqli("172.17.1.51","root","1234","textos");
                    ...
                    if($enlace->connect_errno) {
                      ...
                      if(!$resultado_consulta){
                        ...
                        while($row = $resultado_consulta->fetch_assoc()){
                          echo '<li>'.$row["texto"].' <a href="'.'http://hadoop.ugr.es:14010/php/muestraResultado.php?actualizar='.$row["id"].'">Actualizar</a> <a href="'.'http://hadoop.ugr.es:14010/php/muestraResultado.php?eliminar='.$row["id"].'">Eliminar</a></li>';
                        }
                        echo '</ul>';
                        echo '<form action="'.'http://hadoop.ugr.es:14010'.'" method="'.'get'.'">
                                  <button type="'.'submit'.'">Volver al inicio</button>
                              </form>';
                      }
                    }
