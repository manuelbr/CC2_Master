            ...
            if(isset($_GET['eliminar'])){
              ...
              if($resultado == "bien"){
                echo '<h2>¡El elemento se ha eliminado!<h2>
                      <form action="'.'http://docker.ugr.es:15065/'.'" method="'.'get'.'">
                ...
                if(isset($_GET['actualizar']) && isset($_POST['tex'])){
                  ...
                  if(isset($_GET['actualizar'])){
                    echo '<h2>Actualiza el elemento<h2>
                          <form action="'.'http://docker.ugr.es:15065/php/muestraResultado.php?actualizar='.$_GET['actualizar'].'" method="'.'post'.'">
                    ...
                    if($enlace->connect_errno) {
                      ...
                      if(!$resultado_consulta){
                        ...
                        while($row = $resultado_consulta->fetch_assoc()){
                          echo '<li>'.$row["texto"].' <a href="'.'http://docker.ugr.es:15065/php/muestraResultado.php?actualizar='.$row["id"].'">Actualizar</a> <a href="'.'http://docker.ugr.es:15065/php/muestraResultado.php?eliminar='.$row["id"].'">Eliminar</a></li>';
                        }
                        echo '</ul>';
                        echo '<form action="'.'http://docker.ugr.es:15065/'.'" method="'.'get'.'">
                                  <button type="'.'submit'.'">Volver al inicio</button>
                              </form>';
                      }
                    }
